...
const perform = async (z, bundle) => {
  ...
  try {
    ...
    var key_array = [];
    //only web3 drivers with actoins
    ...
    filtered_array.map((driver) => {
      ...
      if (driver.triggers && driver.triggers.length > 0) {
        key_array.push({
          id: driver.key,
          key: driver.key,
          title: driver.name,
...
module.exports = {
  ...
  key: "list_drivers_with_triggers",
  noun: "List_drivers_with_triggers",

  display: {
    label: "List Drivers with triggers (hidden)",
    description: "List Grindery Nexus Drivers with Triggers",
    hidden: true,
  },
